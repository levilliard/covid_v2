...
export class CovidCrudComponent implements OnInit {
  ...
  findPatient(param: string){
    let data: string = "";
    if(param.includes('signe')){
      data = this.patient;
    }else{
      data = this.firstFormGroup.value["nihid"];
    }


      this.gProvider.getArrayOfObject(param + "/patnetid?id=" + data).subscribe(res=>{
